...
class VisualOdometry:
	...
	def processFrame(self, frame_id):
		 # Step 1: Match tracked feature points between the current and next frame
		kp1 = [cv2.KeyPoint(x[0], x[1], 1) for x in self.px_ref]    # Points from previous frame
		kp2 = [cv2.KeyPoint(x[0], x[1], 1) for x in self.px_cur]    # Points from current frame
		
		# Ensure we have at least 8 points to display
		if len(kp1) < 8 or len(kp2) < 8:
			print("Not enough points to display.")
			return

		# Randomly select 8 points
		selected_indices = random.sample(range(len(kp1)), 8)
		kp1_selected = [kp1[i] for i in selected_indices]
		kp2_selected = [kp2[i] for i in selected_indices]

		# Predefined colors (red, green, blue, yellow, cyan, magenta, orange, purple)
		colors = [
		...
		# 현재 프레임이 흑백이라면 컬러로 변환
		
		if len(self.new_frame.shape) < 3 or self.new_frame.shape[2] == 1:
			self.new_frame = cv2.cvtColor(self.new_frame, cv2.COLOR_GRAY2BGR)

		if len(self.last_frame.shape) < 3 or self.last_frame.shape[2] == 1:
			self.last_frame = cv2.cvtColor(self.last_frame, cv2.COLOR_GRAY2BGR)

		# Create a new image by stacking current and next frames side-by-side for visualization
		height, width = self.new_frame.shape[:2]
		stacked_image = np.hstack((self.last_frame, self.new_frame))

		# Draw selected points and lines on the stacked image
		for i, (p1, p2) in enumerate(zip(kp1_selected, kp2_selected)):
			color = colors[i]  # Use predefined colors in sequence

			# Offset the x-coordinate of the current frame points for the side-by-side effect
			p2_with_offset = (int(p2.pt[0] + width), int(p2.pt[1]))

			# Draw circles on feature points
			cv2.circle(stacked_image, (int(p1.pt[0]), int(p1.pt[1])), 5, color, -1)   # Previous frame points
			cv2.circle(stacked_image, p2_with_offset, 5, color, -1)                   # Current frame points

			# Draw line connecting matched points
			cv2.line(stacked_image, (int(p1.pt[0]), int(p1.pt[1])), p2_with_offset, color, 2)

		# Step 2: Save the image only for a specific random frame pair
		# Run this only once, for example, when frame_id matches a specific condition
		random_frame_id = 5  # Replace this with a specific frame number for a single execution
		if frame_id == random_frame_id:
			output_path = "/home/ircvlab/VIO/feature_matching_output.png"
			cv2.imwrite(output_path, stacked_image)
			print(f"Feature matching visualization saved at: {output_path}")
		...
		if(angle_difference > np.deg2rad(0.0)):
			print("VO updated")
