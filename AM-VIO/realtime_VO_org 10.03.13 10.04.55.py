...
class VisualOdometry:
	...
	def processFrame(self, frame_id):
		...
		if self.skip_frame:
			if self.px_ref.shape[0] < kMinNumFeature:  # Verify if features on last_frame are sparse
				print("sparse tracked feature")
				self.px_cur = self.detectNewFeatures(prev_img)
				self.px_ref = self.px_cur
				self.last_cloud = self.new_cloud
				# self.cur_R = self.Rot.copy()
		...
		if(angle_difference > np.deg2rad(0.03)):
			print("frame updated")
			self.flag = True
	...
	def update(self, img, frame_id, data):
		...
		# self.new_roi = img[int(height * 0.15):int(height * 0.85), int(width * 0.15):int(width * 0.85)]
		self.new_roi = img
